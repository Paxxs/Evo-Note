...
export const DEFAULT_DB_NAME = "evonote-local";
...
interface BinaryDB extends DBSchema {
  collection: {
    key: string;
...
export function upgradeDB(db: IDBPDatabase<BinaryDB>) {
...
export class IndexedDBDocSource implements DocSource {
  name = "indexeddb";
  mergeCount = 1;
  dbPromise: Promise<IDBPDatabase<BinaryDB>> | null = null;
  ...
  getDb() {
    if (this.dbPromise === null) {
      this.dbPromise = openDB<BinaryDB>(this.dbName, dbVersion, {
        upgrade: upgradeDB,
      });
    }
